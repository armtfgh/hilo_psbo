...
def _sanitize_readout(readout: Dict[str, Any]) -> Tuple[Dict[str, Any], List[str]]:
    ...
    if isinstance(effects, dict):
        ...
        effects = {k: v for k, v in effects.items() if isinstance(v, dict) and isinstance(v.get("effect"), str)}
    ro["effects"] = effects
...
with col_chat:
    ...
    if user_msg:
        ...
        system = (
            "You are an expert scientific assistant. Ask ONE focused question at a time "
            "to elicit prior knowledge about the UGI reaction. "
            "Briefly confirm any constraints you will apply (1 sentence), then ask one question. "
            "Keep it brief."
        )
        try:
            followup = _call_openai_chat(
                [{"role": "system", "content": system}] + st.session_state.chat_history,
                model=st.session_state.model,
                temperature=float(st.session_state.temperature),
                api_key=st.session_state.api_key.strip() or None,
            )
    ...
    with perf_col:
        ...
        if hist:
            ...
            if not df_plot.empty:
                chart_df = df_plot.groupby("iter")["best_so_far"].mean().reset_index()
                fig = plt.figure(figsize=(4.6, 3.2))
                ax = fig.add_subplot(111)
                ax.plot(chart_df["iter"] + 1, chart_df["best_so_far"], color="#2c3e50", linewidth=2.0)
