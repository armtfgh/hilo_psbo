...
def _normalize_readout_to_unit_box(
    ...
    *,
    ...
    for b in ro.get("bumps", []) or []:
        ...
        d = int(mins.numel())
        if len(mu_list) < d:
            for j in range(len(mu_list), d):
                mid = float((mins[j] + maxs[j]) / 2.0)
                mu_list.append(mid)
        elif len(mu_list) > d:
            mu_list = mu_list[:d]

        mu_vec = torch.tensor(mu_list, dtype=mins.dtype, device=mins.device)
        mu_norm = ((mu_vec - mins) / rng).clamp(1e-6, 1 - 1e-6)

        if isinstance(sigma, (list, tuple)):
            sigma_list = list(sigma)
            if len(sigma_list) < d:
                for j in range(len(sigma_list), d):
                    sigma_list.append(float(0.15 * rng[j].item()))
            elif len(sigma_list) > d:
                sigma_list = sigma_list[:d]

            sigma_vec = torch.tensor(sigma_list, dtype=mins.dtype, device=mins.device)
